...
export class CargosComponent implements OnInit {
  ...
  getListas() {
    ...
    this.lstClasificacion = varlistas.filter((x: any) => x.nombre_lista == 'BAS_CLASIFICACION');
    this.lstClasificacion.forEach((x: any) => {
      x.id = x.lista_dinamica_id;
      x.detalle = x.lista_dinamica;
    });
  }
  ...
  saveConfiguracion() {
    ...
    let clasificacion_id = Number(this.model.varConfiguracion.clasificacion_id);
    this.model.varConfiguracion.clasificacion_id = clasificacion_id == 0 ? null : clasificacion_id;
    ...
    this.cargo.getCargosConfiguracionId({cargo_configuracion_id: id}).subscribe(data1 => {
      ...
      if (response.tipo == 0) {
        if (response.result.length != 0) {
          ...
          this.model.varConfiguracion.clasificacion_id = cargo.clasificacion_id;
          ...
          this.model.varConfiguracion.clasificacion_id = 0;
  ...
  openInforme(cargo_grado_id: any) {
    this.informeModal = true;
    this.titleCargo = 'Reporte: ' + this.model.cargo;
    this.url = "<iframe src=\"{0}\" width=\"100%\" height=\"600\"><iframe>";
    this.url = this.sanitizer.bypassSecurityTrustHtml(this.url.replace("{0}", this.reporte.getReporteCargoPreview(cargo_grado_id) + "#zoom=100&toolbar=0"));
    this.link = this.reporte.getReporteCargo(cargo_grado_id);
  }
  ...
  exportExcelVHojas() {
    ...
    this.cargo.cargosExportarExcel().subscribe(
      (data: any) => {
  ...
  eliminarCargos(data: any, index: any) {
    ...
    this.cargo.consultarRutasPorCargo(data.cargo_id).subscribe(
      (rutas: any[]) => {
        ...
        mensaje = "Este cargo está asociado a las siguientes rutas:<br />";
        rutas.forEach((ruta: any, idx: number) => {
          mensaje += `${ruta}`;
        ...
        this.cargo.consultarUbicacionesPorCargo(data.cargo_id).subscribe(
          (dato: any[]) => {
            mensaje += "Este cargo está asociado a las Ubicaciones de los siguientes cargos:<br />";
            let isFirst = true;
            let count = Object.keys(dato).length;
            let current = 0;
            ...
            for (const key in dato) {
              if (dato.hasOwnProperty(key)) {
                ...
                mensaje += `Cargo Id: ${key} - Grado: ${dato[key]}`;
            ...
            this.cargo.consultarCargosPorCargo(data.cargo_id).subscribe(
              (cargos: any) => {
                mensaje += "\n\nEste cargo también está asociado a las Experiencias de los siguientes cargos:<br />";
                isFirst = true; // Variable para controlar si es el primer par de valores
                count = Object.keys(cargos).length; // Obtiene la cantidad total de pares de valores
                current = 0; // Variable para llevar el seguimiento del par actual
                ...
                mensaje += "Si eliminas este cargo, automáticamente se eliminará en las rutas, ubicaciones y experiencias asociadas.<br />";
          ...
          (error) => {
            ...
            swal({
              title: 'Error',
              text: 'No se pudieron obtener las ubicaciones asociadas al cargo.',
              allowOutsideClick: false,
              showConfirmButton: true,
              confirmButtonText: 'Aceptar',
              type: 'error'
